...
result <- class::knn(train = wbcd_train, test = wbcd_test, cl = train_labels, k = 21) %>% 
  caret::confusionMatrix(reference = test_labels)
result
```
...
```{r}
class::knn(train = wbcd_train_z[, -1],
                               test = wbcd_test_z[, -1],
                               cl = wbcd_train_z_labels, k = 21) %>% 
  caret::confusionMatrix(reference = wbcd_test_z_labels)
```
...
```{r, message=FALSE}
set.seed(seed)
(knnfit <- e1071::tune.knn(x = wbcd_train, y = train_labels, k = 1:30)) %>% 
  summary()
...
```{r}
set.seed(seed)
caret::train(x = wbcd_train, y = train_labels,
             method = "knn", tuneGrid = expand.grid(k = c(1:30)),
